...
proc loadVersionsFromArchiveCache*(
    nc: var NimbleContext;
    pkg: var Package;
    cachePath: Path
): bool =
  ...
  try:
    ...
    warn pkg.url.projectName, "Unable to load archive cache:", $cachePath, "error:", err.msg
    return false
  pkg.versions.clear()
  for entry in cache.versions:
    let pkgVer = decodeCachedVersionTag(entry, cachePath).toPkgVer()
    ...
    registerReleaseDependencies(nc, pkg, release)
  if pkg.versions.len == 0:
    warn pkg.url.projectName, "Archive cache is missing version history for:", $pkg.url
    return false
  if cache.packageErrors.len > 0:
    pkg.errors = cache.packageErrors
  info pkg.url.projectName, "Loaded", $pkg.versions.len, "releases from archive cache"
  result = true
